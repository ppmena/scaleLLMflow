...
test_that("get_mode works correctly", {
  expect_equal(scaleLLMflow:::get_mode(c(1, 2, 2, 3)), 2)
  expect_equal(scaleLLMflow:::get_mode(c(1, 1, NA, 3)), 1)
  expect_true(is.na(scaleLLMflow:::get_mode(c(NA, NA))))
})
...
test_that("get_item_score parses various score formats", {
  ...
  expect_equal(scaleLLMflow:::get_item_score(1, txt), 1.0)
  expect_equal(scaleLLMflow:::get_item_score(2, txt), 0.5)
  expect_equal(scaleLLMflow:::get_item_score(3, txt), 9.0)
  expect_true(is.na(scaleLLMflow:::get_item_score(4, txt)))
})
...
test_that("extract_ordered_items handles multiline and formats correctly", {
  ...
  parsed <- scaleLLMflow:::extract_ordered_items(agent_output, items = 1:2)
...
test_that("strip_references_section handles safe stripping", {
  ...
  stripped <- scaleLLMflow::strip_references_section(text_with_references)
  ...
  not_stripped <- scaleLLMflow::strip_references_section(text_mid_sentence)
